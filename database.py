...
async def get_inventory_grouped(user_id_tg):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute("SELECT id FROM users WHERE tg_id = ?", (user_id_tg,)) as cursor:
            ...
            if not u: return []
            user_pk = u[0]
        sql = "SELECT i.id as item_id, i.name, i.rarity, i.image_url, i.price, COUNT(inv.item_id) as quantity FROM inventory AS inv JOIN items AS i ON inv.item_id = i.id WHERE inv.user_id = ? GROUP BY i.id, i.name, i.rarity, i.image_url, i.price"
        async with db.execute(sql, (user_pk,)) as cursor: return await cursor.fetchall()
